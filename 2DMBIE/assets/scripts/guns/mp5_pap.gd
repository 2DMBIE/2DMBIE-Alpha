...
func _init():
	...
	texture = load("res://assets/sprites/guns/mp5_puck_a_panch.png") # The path of the sprite gun.

	bulletpoint = Vector2(41, -8.312) # Position of the bulletpoint.
	bulletdelay = float(.11) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(260), float(750), "res://assets/scenes/bullet.tscn", int(2)) 
