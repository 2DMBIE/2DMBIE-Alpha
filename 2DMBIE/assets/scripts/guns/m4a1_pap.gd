...
func _init():
	...
	texture = load("res://assets/sprites/guns/m4a1_puck_a_panch.png") # The path of the sprite gun.

	bulletpoint = Vector2(69.339,-8.287) # Position of the bulletpoint.
	bulletdelay = float(.15) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(320), float(1250), "res://assets/scenes/bullet.tscn", int(5)) 
