...
func _init():
	...
	bulletdelay = float(.13) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(50), float(1250), "res://assets/scenes/bullet.tscn") 
	...
	camera_decay = float(0.7) # How quickly the shaking of the camera stops [0, 1]. (can be higher than 1 but not lower then zero)
