...
func _init():
	...
	texture = load("res://assets/sprites/guns/spas12.png") # The path of the sprite gun.

	bulletpoint = Vector2(59.753,-8.312) # Position of the bulletpoint.
	bulletdelay = float(1) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(100), float(750), "res://assets/scenes/bullet.tscn") 
