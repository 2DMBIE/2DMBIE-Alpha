...
class_name KAR98K_pap

func _init():
	name = "Kar98kpap" # The name of the gun.
	offset = Vector2(7.994,-1.09) # The position of the gun.
	scale = Vector2(1,1) # The scale of the gun. Default Scale 1 on 1: Vector2(1,1)
	texture = load("res://player/weapons/kar98k_upgraded/kar98k_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(68.153, -6.719) # Position of the bulletpoint.
	bulletdelay = float(1.25) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(1300), float(1700), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn", int(5)) 
	...
	maxclipAmmo = 10 # How much bullets are in one magazine.
	totalAmmo = 90 # Total ammo which comes with each gun.
	ammo = maxclipAmmo
	reload_time = float(4)
