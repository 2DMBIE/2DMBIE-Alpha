...
func _init():
	...
	texture = load("res://player/weapons/m249_upgraded/m249_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(85.171, -14.423) # Position of the bulletpoint.
	bulletdelay = float(.12) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(240), float(1000), "res://player/weapons/all_bullet_scenes/laser_pink/bullet_laser_pink.tscn", int(4)) 
