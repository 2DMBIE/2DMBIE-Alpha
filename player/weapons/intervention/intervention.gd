...
func _init():
	...
	texture = load("res://player/weapons/intervention/intervention.png") # The path of the sprite gun.

	bulletpoint = Vector2(112.888, -10.474) # Position of the bulletpoint.
	bulletdelay = float(2) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(1400), float(1700), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn", int(7)) 
	...
	maxclipAmmo = 7 # How much bullets are in one magazine.
	totalAmmo = 49 # Total ammo which comes with each gun.
	ammo = maxclipAmmo
	reload_time = float(4)
