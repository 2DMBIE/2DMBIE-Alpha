...
func _init():
	...
	texture = load("res://player/weapons/p90/p90.png") # The path of the sprite gun.

	bulletpoint = Vector2(37.162, -5.722) # Position of the bulletpoint.
	bulletdelay = float(.10) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(130), float(750), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn" , int(2)) 
