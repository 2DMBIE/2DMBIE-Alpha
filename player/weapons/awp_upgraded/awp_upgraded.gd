...
func _init():
	...
	texture = load("res://player/weapons/awp_upgraded/awp_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(124.47, -6.613) # Position of the bulletpoint.
	bulletdelay = float(2) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(2800), float(1700), "res://player/weapons/all_bullet_scenes/sniper_bullet/sniper_bullet.tscn", int(7)) 
