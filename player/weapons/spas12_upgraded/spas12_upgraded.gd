...
func _init():
	...
	texture = load("res://player/weapons/spas12_upgraded/spas12_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(61.589,-7.527) # Position of the bulletpoint.
	bulletdelay = float(1.5) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(6000), float(850), "res://player/weapons/all_bullet_scenes/shotgun_pellet_red/shutgun_pellet_red.tscn", int(2)) 
