...
func _init():
	...
	texture = load("res://player/weapons/p90_upgraded/p90_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(37.162, -5.722) # Position of the bulletpoint.
	bulletdelay = float(.10) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(260), float(750), "res://player/weapons/all_bullet_scenes/laser_blue/bullet_laser_blue.tscn", int(2)) 
	...
	totalAmmo = 400 # Total ammo which comes with each gun.
	ammo = maxclipAmmo
	reload_time = float(2)
