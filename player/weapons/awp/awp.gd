...
func _init():
	...
	texture = load("res://player/weapons/awp/awp.png") # The path of the sprite gun.

	bulletpoint = Vector2(124.47, -6.613) # Position of the bulletpoint.
	bulletdelay = float(2) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(1250), float(1700), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn", int(7)) 
	...
	totalAmmo = 40 # Total ammo which comes with each gun.
	ammo = maxclipAmmo
	reload_time = float(4)
