...
func _init():
	...
	texture = load("res://player/weapons/ump45_upgraded/ump45_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(53.979, -8.294) # Position of the bulletpoint.
	bulletdelay = float(.12) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(340), float(750), "res://player/weapons/all_bullet_scenes/laser_green/bullet_laser_green.tscn", int(2)) 
