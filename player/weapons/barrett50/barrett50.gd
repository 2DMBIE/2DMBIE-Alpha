...
func _init():
	...
	texture = load("res://player/weapons/barrett50/barrett50.png") # The path of the sprite gun.

	bulletpoint = Vector2(79.424, -7.527) # Position of the bulletpoint.
	bulletdelay = float(1) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(700), float(1700), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn" , int(6)) 
