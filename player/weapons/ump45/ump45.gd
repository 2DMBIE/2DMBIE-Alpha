...
func _init():
	...
	texture = load("res://player/weapons/ump45/ump45.png") # The path of the sprite gun.

	bulletpoint = Vector2(53.979, -8.294) # Position of the bulletpoint.
	bulletdelay = float(.12) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(170), float(750), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn", int(2)) 
	...
	maxclipAmmo = 24 # How much bullets are in one magazine.
	totalAmmo = 192 # Total ammo which comes with each gun.
	ammo = maxclipAmmo
	reload_time = float(2)
