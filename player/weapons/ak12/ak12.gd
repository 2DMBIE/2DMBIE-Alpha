...
func _init():
	...
	texture = load("res://player/weapons/ak12/ak12.png") # The path of the sprite gun.

	bulletpoint = Vector2(55.659, -6.042) # Position of the bulletpoint.
	bulletdelay = float(.18) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(210), float(1000), "res://player/weapons/all_bullet_scenes/normal_bullet/bullet.tscn", int(5)) 
