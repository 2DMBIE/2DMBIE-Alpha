...
func _init():
	...
	texture = load("res://player/weapons/m60_upgraded/m60_upgraded.png") # The path of the sprite gun.

	bulletpoint = Vector2(107.246, -12.552) # Position of the bulletpoint.
	bulletdelay = float(.25) # The delay between each bullet. [0.1, 0.2]

	_bullet = Bullet.new(float(500), float(1000), "res://player/weapons/all_bullet_scenes/laser_blue/bullet_laser_blue.tscn", int(4)) 
